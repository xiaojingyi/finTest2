...
class Runner(App):
    ...
    def backUptestCallback(self, blobs, bsize):
        res = []
        last_walk = 0
        for j in range(bsize):
            ...
            walk = sum(blobs["label_"].data[j])
            """
            l_walk = one - last_walk
            """
            l_walk = one
            if l_walk == 0:
                label = 0
            ...
            print last_walk
            print one
            print label, int(sum(label_real)), sum(walk)
            exit()
            """
            res.append([label, int(sum(label_real)), walk-last_walk])
            last_walk = walk
