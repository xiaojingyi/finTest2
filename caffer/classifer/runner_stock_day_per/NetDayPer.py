...
class NetDayPer(object):
    def __init__(self, config):
        if not config:
            self.bail(-1, "no config: NetDayPer init")
        self.config = config
        self.debug = config["debug"]
        #super(NetDayPer, self).__init__(config)
    ...
    def testNet(self, margin=1):
        ...
        #data_drop = slices[1]
        conv1 = net.convLayer(data_drop, [one_len, 1], [one_len, 1], 32, replace="relu")
        layer = net.normLayer(conv1)
        #layer = net.batchNormLayer(conv1)
        pool1 = net.poolLayer(layer, [6, 1], [2, 1], "max", pad_wh=[2, 0])
        conv2 = net.convLayer(pool1, [4, 1], [2, 1], 16, pad_wh=[1, 0], replace="relu")
        conv3 = net.convLayer(conv2, [4, 1], [2, 1], 8, pad_wh=[1, 0], replace="relu")
        flat = net.flattenLayer(conv3)

        concat = net.concatLayer(*[fc, flat])

        helper1 = net.fcLayer(concat, 512, replace='relu', dropout=self.dropout)
        helper1_2 = net.fcLayer(helper1, 512, replace='relu', dropout=self.dropout)
    ...
    def testNetSim(self, single_dim, stock_dim, num, margin=1):
        ...
        conv1 = net.convLayer(slices[1], [5, 1], [5, 1], 64, wname=["conv1_w", "conv1_b"], lr=[1, 2])
        layer = net.normLayer(conv1)
