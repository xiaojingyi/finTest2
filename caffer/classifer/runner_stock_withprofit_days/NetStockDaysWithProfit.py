...
# FileName: NetStockDaysWithProfit.py
...
class NetStockDaysWithProfit(object):
    def __init__(self, config):
        if not config:
            self.bail(-1, "no config: NetStockDaysWithProfit init")
        self.config = config
        self.debug = config["debug"]
        #super(NetStockDaysWithProfit, self).__init__(config)
