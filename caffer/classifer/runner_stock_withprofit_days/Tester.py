...
class Tester(App):
    ...
    def mkNet(self, margin=1, dropout=0.5):
        ...
        netname = f.init("NetStockDaysWithProfit", {"margin": margin, "dropout": dropout}, "gpu%d_" % self.gpu_id + "alex.prototxt")
        return netname
    ...
    def run(self):
        ...
        self.runBase(queue)
        self.testSteps(str(self.config['model']), True)
        self.testSteps(str(self.config['model']), True)
        self.testSteps(str(self.config['model']), True)
...
def main():
    conf = {
            ...
            "batch_len": 256 * 1,
            "dcache_prefix": "alex",
            "data_type": "DataStockDaysWithProfit",
            "is_sim": False,
            }
    t = Tester(conf)
    t.run()
