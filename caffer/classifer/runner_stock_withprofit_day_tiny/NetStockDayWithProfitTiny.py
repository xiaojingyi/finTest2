...
# FileName: NetStockDayWithProfitTiny.py
...
class NetStockDayWithProfitTiny(object):
    def __init__(self, config):
        if not config:
            self.bail(-1, "no config: NetStockDayWithProfitTiny init")
        self.config = config
        self.debug = config["debug"]
        #super(NetStockDayWithProfitTiny, self).__init__(config)
...
def main():
    ...
    t = NetStockDayWithProfitTiny(conf)
    t.testPrint()
    return
