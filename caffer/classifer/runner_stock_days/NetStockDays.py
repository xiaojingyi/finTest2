...
# FileName: NetStockDays.py
# Date: 2016 Sat 09 Jul 2016 12:02:01 AM CST
# Encoding: utf-8
...
class NetStockDays(object):
    def __init__(self, config):
        if not config:
            self.bail(-1, "no config: NetStockDays init")
        self.config = config
        self.debug = config["debug"]
        #super(NetStockDays, self).__init__(config)
        self.one_dim = 4 * 10
    ...
    def testNet(self, margin=1):
        ...
        conv1 = net.convLayer(drop, [self.one_dim, 1], [self.one_dim, 1], 64)
