...
class CaffeTrainerCmd(object):
    def __init__(self, config):
        ...
        self.data_gen_dir = config['cache_dir']
        ...
        while True:
            ...
            if self.is_gening_data == False:
                if self.is_copying == False and self.is_data == False:
                    ...
                    self.is_data = True
                    i += 1
                    if i == int(math.ceil(self.data_len * 1.0 / self.batch_size)):
                        i = 0
                    np.save(ifile, i)
            time.sleep(1)
        return
    ...
    def mkSolverPrototxt(self, i):
        ...
        content = fp.read()
        fp.close()
        content = content.replace("{max_iter}", str((i+1) * int(self.batch_size / self.train_batch_size + 1)))
    ...
    def trainIter(self, time_iter):
        ...
        if time_iter > 0 and time_iter % self.test_iter == 0:
            ...
            cmd = "caffe test -model %s -weights %s -iterations 100 -gpu all  2>&1 | grep -E \"solver.cpp|caffe.cpp\" >> test.log" % (self.model_prototxt, model)
            self.call(cmd)
